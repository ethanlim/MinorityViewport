	...
	void MinorityViewport::MergeScenes(){
			while(1){
				...
				if(orderedScenes.size()>0){
					for(vector<Scene*>::iterator orderedSceneItr = orderedScenes.end();orderedSceneItr!=orderedScenes.begin();orderedSceneItr--){
						...
						if((*orderedSceneItr)->GetCalibration()){
							...
							if(leftScenePtr==NULL||leftFrameSkeletons.size()==0){
